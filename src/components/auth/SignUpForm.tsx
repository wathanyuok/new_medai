...
export default function SignUpForm() {
  ...
  const [usePrefix, setPrefix] = useState<string>("");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const confirmEmail = (form.elements.namedItem('confirmEmail') as HTMLInputElement).value.trim();
    const phone = (form.elements.namedItem('phone') as HTMLInputElement).value.trim();
    const password = (form.elements.namedItem('password') as HTMLInputElement).value;
    const confirmPassword = (form.elements.namedItem('confirmPassword') as HTMLInputElement).value;

    if (!email || !confirmEmail || !phone || !password || !confirmPassword) {
      toast.error("กรุณาระบุข้อมูลให้ครบทุกช่องที่มี *");
      return;
    }

    if (email !== confirmEmail) {
      toast.error("อีเมลและยืนยันอีเมลไม่ตรงกัน");
      return;
    }
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                  ...
                  <div>
                    <Label>ยืนยันอีเมล <span className="text-error-500">*</span></Label>
                    <Input
                      type="email"
                      name="confirmEmail"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <span className="text-brand-500 hover:text-brand-600 dark:text-white/90 font-medium">
                      <Link href="">ข้อตกลงการใช้บริการ</Link>
                    </span>{" "}
                    และ{" "}
                    <span className="text-brand-500 hover:text-brand-600 dark:text-white/90 font-medium">
