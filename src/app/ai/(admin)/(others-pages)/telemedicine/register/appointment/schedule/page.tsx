...
// ✅ Helper function แปลง Date เป็น string แบบ local timezone (ไม่ใช้ UTC)
...
const SchedulePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadWorkingSchedule = async () => {
      ...
      // ✅ เช็คเวลาปัจจุบันสำหรับกรองวันนี้
      const now = new Date();
      ...
      slots.forEach((slot) => {
        if (slot.date_string) {
          ...
          if (slot.time_string) {
            ...
            // ✅ ถ้าเป็นวันนี้ → เช็คว่าเวลาผ่านไปแล้วหรือยัง
            if (slot.date_string === todayStr && isAvailable) {
              const [hours, minutes] = slot.time_string.split(':').map(Number);
              if (hours < currentHour || (hours === currentHour && minutes <= currentMinute)) {
                isAvailable = false; // เวลาผ่านไปแล้ว
              }
            }
          ...
          // ✅ เช็คว่ามีเวลาว่างจริงๆ หลังกรองเวลาที่ผ่านไปแล้ว
          const availableSlots = timesByDate.get(slot.date_string)?.filter(s => s.isAvailable) || [];
          if (availableSlots.length > 0) {
            availableDatesSet.add(slot.date_string);
          } else {
            availableDatesSet.delete(slot.date_string);
          }
        }
      });
  ...
  // ✅ Helper: ตรวจสอบว่าเวลานี้ผ่านไปแล้วหรือยัง (สำหรับวันนี้)
  ...
  useEffect(() => {
    if (selectedDate) {
      // ✅ ใช้ formatDateToString แทน toISOString()
      const dateStr = formatDateToString(selectedDate);
      let slots = timeSlotsByDate.get(dateStr) || [];
      
      // ✅ ถ้าเป็นวันนี้ → กรองเวลาที่ผ่านไปแล้วออก (mark as unavailable)
      const isToday = formatDateToString(selectedDate) === formatDateToString(new Date());
      if (isToday) {
        slots = slots.map(slot => ({
          ...slot,
          isAvailable: slot.isAvailable && !isTimePassed(slot.time)
        }));
      }
  ...
  // ✅ Check if date is in schedule (มีในตารางหมอ) - ใช้ local timezone
  ...
  // ✅ Check if date has available slots (มีเวลาว่างอย่างน้อย 1 slot) - ใช้ local timezone
  const isDateAvailable = (date: Date) => {
    const dateStr = formatDateToString(date);
    
    // ถ้าไม่มีในรายการวันที่ว่าง → ไม่ว่าง
    if (!availableDates.has(dateStr)) return false;
    
    // ✅ ถ้าเป็นวันนี้ → ต้องเช็คว่ามีเวลาที่ยังไม่ผ่านไปด้วย
    ...
    if (isToday) {
      ...
      // เช็คว่ามีเวลาที่ยังไม่ผ่านและยังว่างอยู่หรือไม่
      const hasFutureAvailableSlot = slots.some(slot => {
    ...
    <div className="min-h-screen bg-[#F2F8FE] p-4">
      ...
      {selectedDate && (
        <div className="bg-white rounded-2xl p-4 shadow-sm mt-4">
          ...
          {timeSlotsForDate.filter(s => s.isAvailable).length > 0 ? (
            <div className="relative">
              ...
              {showTimeDropdown && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-10 max-h-48 overflow-y-auto">
                  {timeSlotsForDate
                    .filter((slot) => slot.isAvailable) // ✅ แสดงเฉพาะเวลาที่ว่างเท่านั้น
                    .map((slot) => (
                    <button
                      key={slot.time}
                      onClick={() => {
