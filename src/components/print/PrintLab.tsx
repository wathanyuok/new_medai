...
export default function MultiPDFMergePage(queue_id: any) {
  ...
  // Ref สำหรับเก็บ window ที่เปิดไว้ก่อน (สำหรับ iOS) — ตอนนี้ไม่ได้ใช้ก็ไม่เป็นไร
  ...
  useEffect(() => {
    const autoProcessPDF = async () => {
      if (dataLoaded && !autoProcessed && printData && Object.keys(printData).length > 0) {
        ...
        // Android ใช้โค้ดนี้อยู่แล้ว
        // iOS เวอร์ชันจริงคุณแยกไฟล์ / แยกหน้าอยู่แล้ว ไม่เกี่ยวตัวนี้
  ...
  const mergePDFs = async (shouldAutoOpen: boolean = true) => {
    if (s3Urls.length === 0) {
      ...
      if (isMobile && shouldAutoOpen) {
        if (isIOS) {
          // อันนี้ถ้าเอาไปใช้ iOS ด้วยก็จะเปิดแท็บใหม่
          openPDFOnIOS(url);
        } else {
  ...
  // นับจำนวนลิงค์และแยกประเภท (เผื่อไว้)
        ...
        <div className="bg-white shadow-lg rounded-lg p-3 sm:p-6 mt-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 space-y-3 sm:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
              {/* ✅ ปุ่ม Download — แสดงทุก platform */}
              <button
              ...
              {/* ✅ ปุ่ม Open PDF — แสดงทุก platform */}
              {isMobile && (
                <button
              ...
              {/* ✅ ปุ่ม Lab Only + ส่วนด้านล่าง แสดงเฉพาะ Desktop หรือ iOS เท่านั้น */}
              {(!isMobile || isIOS) && (
                <button
          ...
          {/* ❗ ตั้งแต่ตรงนี้ลงไป: ซ่อนบน Android (isMobile && !isIOS) */}
          {(!isMobile || isIOS) && (
            <>
              {/* Mobile Warning (จะเหลือใช้บน iOS มือถือเท่านั้น เพราะ Android ถูกกันออกด้วยเงื่อนไขด้านนอก) */}
              {isMobile && (
                <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-amber-800 text-sm">
                      ...
                      <p className="mt-1">
                        กดปุ่ม <strong>"เปิดใน Tab ใหม่"</strong> หรือ{' '}
                        <strong>"ดาวน์โหลด PDF"</strong> เพื่อดูไฟล์
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <object data={`${previewUrl}#toolbar=1&navpanes=1&scrollbar=1`} type="application/pdf" width="100%" height="600px">
                      <iframe src={`${previewUrl}#toolbar=1&navpanes=1&scrollbar=1`} width="100%" height="600px">
                        <div className="p-8 text-center bg-gray-100">
                          ...
                          {/* ปุ่ม fallback นี้จะไม่โผล่บน Android เพราะ wrapper ด้านบนกันไว้แล้ว */}
                          <button
                            onClick={openInNewTab}
              ...
              <div className="mt-3 p-2 sm:p-3 bg-gray-50 rounded text-xs sm:text-sm text-gray-600">
                ...
                <span className="block sm:inline sm:ml-1">
                  ใช้ toolbar ของ viewer เพื่อเลื่อนหน้า ซูม พิมพ์ หรือ ดาวน์โหลดไฟล์ PDF
                </span>
              </div>
            </>
          )}
